...
<head>
    ...
    <title>갤러리 매출 관리 - Gallery nur</title>
</head>
<style>
    table {
    ...
    <div class="container">
        <ul class="nav justify-content-center" id="gry">
            <li class="nav-item col-2">
                <span class="nav-link" aria-current="page" id="cgv" style="cursor:hand;"
                      onclick=document.location.href="/exhibit/upload">전시회 추가</span>
            </li>
            <li class="nav-item col-2">
                <span class="nav-link" aria-current="page" id="mmt" style="cursor:hand;"
                      onclick=document.location.href="/artist/upload">작가 추가</span>
            </li>
            <li class="nav-item col-2">
                <span class="nav-link" aria-current="page" id="gallerySaleForNow" style="cursor:hand;">전시회 매출</span>
            </li>
            <li class="nav-item col-2">
                <span class="nav-link" aria-current="page" id="galleryPeopleForNow" style="cursor:hand;">전시회 총 입장수</span>
            </li>
            <li class="nav-item col-2">
                <span class="nav-link" aria-current="page" id="gallerySaleForYear" style="cursor:hand;">지난 1년 매출</span>
            </li>
        </ul>
    </div>
    <br>
...
<script>
    ...
    function loadSaleForNow() {
        let temp = `<canvas id = "Chart" class = "container col-12 text-center">

                        </canvas>`
        $("#chartSection").append(temp);
        $.ajax({
            type: "POST",
            url: "/management/nowsale",
            data: "",
            dataType: "JSON",
...
<script>
    $(document)
        .on("click", "#gallerySaleForNow", function () {
            $("#Chart").remove();
            loadSaleForNow();
        })
        .on("click", "#gallerySaleForYear", function () {
            $("#Chart").remove();
            let temp = `<canvas id = "Chart" class = "container col-12 text-center">
        ...
        .on("click", "#galleryPeopleForNow", function () {
            $("#Chart").remove();
            let temp = `<canvas id = "Chart" class = "container col-12 text-center">
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            let temp = data[i];
                            name.push(temp['name']);
                            people.push(temp['people']);
                            peopleSum += parseInt(temp['people']);
                            colorList.push(colorize());
                        }
                        name.push("총 인원");
                        people.push(peopleSum);
                        colorList.push(colorize());
                        let saleChart = {
                            labels : name,
                            datasets : [{
                                label : '현재 전시중인 전시회 총 입장 수',
                                data : people,
                                backgroundColor : colorList
                            }]
                        };
