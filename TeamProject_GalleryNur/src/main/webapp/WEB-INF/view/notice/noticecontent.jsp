...
<head>
    ...
    />
    <title>${ndto.title} - Gallery nur</title>
</head>
<style>
    table {
        ...
        <div class="col-3 text-start">
            <c:if test="${nepr.next!=9999}">
                <button type="button" class="btn btn-outline-secondary"
                        onclick="location.href='content?id=${nepr.next}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}'">
                    이전
                </button>
            </c:if>
        </div>
        <div class="col-6 text-center">
            <c:if test="${user.id != null and user.id != ndto.userid}">
                <button type="button" class="btn btn-outline-Danger" onclick="updateLike(); return false;">추천</button>
            </c:if>
            <c:if test="${user.id == null or user.id == ndto.userid}">
                <button type="button" class="btn btn-outline-Danger" disabled>추천</button>
            </c:if>
            <button type="button" class="btn btn-outline-dark" onclick="location.href='/notice'">목록</button>
        </div>
        <div class="col-3 text-end">
            <c:if test="${nepr.prev!=0}">
                <button type="button" class="btn btn-outline-secondary"
                        onclick="location.href='content?id=${nepr.prev}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}'">
                    다음
                </button>
            </c:if>
        </div>
