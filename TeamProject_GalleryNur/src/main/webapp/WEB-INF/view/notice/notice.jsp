...
<style>
    ...
    /*	src: url("./static/Spartan_Thin.woff") format('woff');*/
    /*}*/
    table {
...
<main class="container p-5">
    ...
    <div class="container" id="boardList">
        <div class="row">
                    ...
                    <c:when test="${!empty listnotice}">
                        <c:forEach items="${listnotice}" var="list">
                            <tr class="text-center">
                                <td>${list.id}</td>
                                <td style="width:60%;"><a
                                        href="content?id=${list.id}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}">${list.title}</a>
                                </td>
                                <td>${list.userid}</td>
                                <td>${list.postdate}</td>
                                <td>${list.views}</td>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item">
                        <a class="page-link"
                           href="notice?curPage=1&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}"
                           aria-label="Previous">
                    ...
                    <c:forEach begin="${paging.firstPage}" end="${paging.lastPage}" var="i">
                        <li class="page-item"><a class="page-link"
                                                 href="notice?curPage=${i}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}">
                    ...
                    <li class="page-item">
                        <a class="page-link"
                           href="notice?curPage=${paging.totalPageCount}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}"
                           aria-label="Next">
        ...
        <c:if test="${user.role=='관리자'}">
            <div class="col-12 text-end">
                <button type="button" id=btnwrite class="btn btn-outline-dark btn-sm">글쓰기</button>
            </div>
        </c:if>
    </div>
    </div>
