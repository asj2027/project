...
<head>
    ...
    />
    <title>${rdto.title} - Gallery nur</title>
</head>
<style>
    .px-2:hover {
        cursor: pointer;
...
<main class="container p-5">
        ...
        <div class="col-3 text-start">
            <c:if test="${ndto.next != 9999}">
                <button type="button" class="btn btn-outline-secondary"
                        onclick="location.href='reviewDetail?id=${ndto.next}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}'">
                    이전
                </button>
            </c:if>
        </div>
        <div class="col-6 text-center">
            <c:if test="${user.id != null and user.id != rdto.userid}">
                <button type="button" class="btn btn-outline-Danger" onclick="updateLike(); return false;">추천</button>
            </c:if>
            <c:if test="${user.id == null or user.id == rdto.userid}">
                <button type="button" class="btn btn-outline-danger" disabled>추천</button>
            </c:if>
            <button type="button" class="btn btn-outline-dark" onclick="location.href='/review'">목록</button>
        </div>
        <div class="col-3 text-end">
            <c:if test="${ndto.last != 0}">
                <button type="button" class="btn btn-outline-secondary"
                        onclick="location.href='reviewDetail?id=${ndto.last}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}'">
                    다음
                </button>
            </c:if>
        </div>
    ...
    <div class="text-end">
        <c:if test="${user.id == rdto.userid || user.role =='관리자'}">
            <button type="button" class="btn btn-outline-Dark" onclick="location.href='reviewUpdate?id=${rdto.id}'">수정
            </button>
            <a href="deleteReview?id=${rdto.id}">
                <button type="button" class="btn btn-outline-Dark" id="deleteB">삭제</button>
            </a>
        </c:if>
    </div>
