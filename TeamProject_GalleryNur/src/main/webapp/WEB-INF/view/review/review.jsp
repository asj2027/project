...
<head>
    ...
    <title>리뷰 게시판 - Gallery nur</title>
</head>
<style>
    table {
...
<main class="container p-5">
    ...
    <form id="serform">
        <div class="col-12 text-end">
            ...
            <input style="width:15%;" type="text" placeholder="검색어를 입력하세요" aria-label=".form-control-sm example"
    ...
    <div class="container" id="boardList">
        <div class="row">
                    ...
                    <c:when test="${!empty reviewlist}">
                        <c:forEach items="${reviewlist}" var="list">
                            <tr class="text-center">
                                <td>${list.id}</td>
                                <td style="width:60%;"><a
                                        href="reviewDetail?id=${list.id}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}">${list.title}</a>
                                </td>
                                <td>${list.userid}</td>
                                <td>${list.postdate}</td>
                                <td>${list.views}</td>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <li class="page-item">
                        <a class="page-link"
                           href="review?curPage=1&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}"
                           aria-label="Previous">
                    ...
                    <c:forEach begin="${paging.firstPage}" end="${paging.lastPage}" var="i">
                        <li class="page-item"><a class="page-link"
                                                 href="review?curPage=${i}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}">
                    ...
                    <li class="page-item">
                        <a class="page-link"
                           href="review?curPage=${paging.totalPageCount}&sort=${paging.sort}&type=${paging.type}&keyword=${paging.keyword}"
                           aria-label="Next">
        ...
        <div class="col-12 text-end">
            <button type="button" class="btn btn-outline-dark btn-sm" onclick="location.href='/writeReview'">글쓰기
            </button>
        </div>
    </div>
    </div>
